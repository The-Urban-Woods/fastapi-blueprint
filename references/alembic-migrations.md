...
For PostgreSQL enums, use `alembic-postgresql-enum`. PostgreSQL enum types are schema objects separate from the table column that uses them, and the extension handles enum create/drop operations plus `op.sync_enum_values(...)` generation for enum value changes.

Install the extension and import it in `migrations/env.py`:
...
With that import in place, `alembic revision --autogenerate` can generate enum create/drop operations and `op.sync_enum_values(...)` calls for enum value changes.

Rules:

- Use `alembic-postgresql-enum` for PostgreSQL projects instead of hand-maintained enum lifecycle code
- Review generated enum migrations before applying them, especially rename scenarios and data-dependent changes
- Do not rewrite old revisions just to make enum handling cleaner.
- If an old migration created or used an enum but never dropped it on downgrade, create a new corrective migration if you need to clean up or rename that enum state.
- If an old migration used inline enum handling and autogenerate got it wrong, leave that revision as-is if it is already in history. Enable `alembic_postgresql_enum` and let the next migration establish the correct generated pattern.
